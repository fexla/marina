...
- **隐藏顶部标签栏后的交互修复。** 此前开启「隐藏顶部标签栏」后,顶部 TabBar 整块移除,连带带走了 `+ 新建` / `× 关闭` 两个常驻入口,且关闭当前终端后主区直接掉进「新建终端」页(即使同目录还有别的终端)。现:
  - 主区补一个极简常驻工具栏(「新建 / 关闭」),hideTopTabBar 模式下可见,就地新建 / 关闭不再依赖侧栏。
  - 「关闭」关掉当前终端后,若同目录有无主(orphan)的终端,自动接管并续看到它;没有才进新建页。
  - 双击侧栏目录 / EmptyPathState 模板按钮新建终端时不再闪一下「新建终端」页(invoke 返回早于 `evt:session:created` 广播时,旧逻辑选中的 id 尚未进入 state 导致的空窗)。
