...
- **收藏分组可递归嵌套（子组）。**
  ...
  拖拽语义为标准树：组标题整行可拖（不增加专用 grip icon），纵向位置决定顺序、
  横向每移动一档决定最终层级；落点线按最终缩进对齐并明确显示目标父级/同级锚点。
  可精确把深层子组提升一级或直接提升到根级；禁止拖入自身或后代（循环守卫）。
  解散分组时路径与子组提升到上级，绝不删数据。
  磁盘 schema 升级为 bookmarks.json v3（v1/v2 启动期自动迁移）。

- **侧栏分组/路径/终端菜单按用户任务重设计。**
  分组动作全部收敛到 ⋯/右键（新建子组、重命名、解散分组），去掉 hover 快捷
  按钮；组头聚焦时 F2 重命名、Delete 解散。路径菜单分「打开/定位、复制、
  组织、启动方式、项目工具」区，默认模板收进子菜单并新增「跟随全局默认」；
  「移动到分组」子菜单可把路径移入任意层级分组或未分组。终端菜单主任务 +「复制
  信息」子菜单（当前目录/初始目录/PID）。远程 backend / SSH 上下文不再显示
  「在文件管理器中显示」（会在用户看不到的电脑上打开）。
...
- **收藏子组拖动补齐精准层级控制，local/SSH 混合收藏不再被后端拒绝。**
  分组块此前只注册 droppable、不参与排序；初版修复又用“上半同级/下半嵌套”猜
  层级，三层以上无法精确选择提升一级还是提升到根级。现在组标题整行可拖，纵向
  决定顺序、横向决定最终深度，并实时显示如“「A」内 · 与「B」同级”的落点文案。
  拖动布局始终基于 backend 全量收藏，不再因遗漏隐藏 segment pathId 被
  `InvalidOrderList` 拒绝。
...
- **SSH 段「+」不再手输路径。**
  收藏与临时「+」统一为「连接 SSH…」：选服务器后从 home（或 profile 默认远端
  目录）开 session，到达目录后从临时/最近加入收藏。
