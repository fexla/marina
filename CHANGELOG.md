...
- **DiffViewer 横向滚动时行号列挡不住代码。** diff 视图左右拖动查看长代码行时,行号列(`.diff-line-gutter`,`sticky left:0` 钉在左边)本应挡住滚过来的代码,但 add/del/hunk 行与查找聚焦行的底色用 `color-mix(..., transparent)` 叠在透明底上,`.diff-line-gutter` 的 `background: inherit` 继承到的是半透明色,挡不住 → 代码文字穿透行号列、和行号重叠,视觉上「行号那一列被代码盖乱」。改为把这些行底色的混合底从 `transparent` 换成不透明的 `var(--color-bg-primary)`(视觉浓度几乎不变),行底不透明后 gutter 即可正确遮挡。TextViewer 的行底色本就不透明,未受影响;`.file-text-line.search-current` 与 diff 共享规则一并修好。
