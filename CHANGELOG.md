...
- **收藏分组可递归嵌套（子组）。**
  ...
  拖拽语义为标准树：拖到组行上半 = 同级排序，下半 = 成为子组；禁止拖入
  自身或后代（循环守卫）；解散分组时路径与子组提升到上级，绝不删数据。
...
- **收藏子组无法拖动、local/SSH 混合收藏拖动被后端拒绝。**
  分组块此前只注册 droppable、不参与排序；且拖动只提交当前 segment 可见路径，
  `BOOKMARK_REORDER` 因遗漏隐藏 segment 的 pathId 被 `InvalidOrderList` 拒绝。
  现在分组整块可排序/嵌套，布局计算始终基于 backend 全量收藏。
