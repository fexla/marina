...
调查定位的根因(GitService 后台每 3s 跑 `git status`、抢 `.git/index.lock` 干扰外部 `git commit`)**已根治**——所有 git 子进程统一注入 `GIT_OPTIONAL_LOCKS=0`,不再持锁、不再写 `.git/index`,同时避免把全局选项错放到 status 参数末尾导致 exit 129 / 面板误报干净。
...
→ 所以这不只是「性能/锁竞争」问题,而是**原则性违规**。最终统一用 `GIT_OPTIONAL_LOCKS=0`:消除锁竞争 + 恢复契约合规 + 避免命令行全局选项的位置错误。

## 改了什么

| 文件 | 改动 |
|---|---|
| `src/main/git-service.ts` | `runGit` 统一注入 env `GIT_OPTIONAL_LOCKS=0`;三处 status 不再携带错误位置的 `--no-optional-locks` |
| `src/main/git-service.test.ts` | 回归断言:status args 不含错误 flag;`buildGitSpawnEnv` 继承父环境并强制 optional locks=0 |
| `CHANGELOG.md` | `[Unreleased]` 段记一条(**不 bump 版本**,符合附录 E:dev 未分发) |
| `docs/软件定义书.md` | ADR-017 安全边界加「实现纪律」注,钉死「git 子进程必须注入 `GIT_OPTIONAL_LOCKS=0`」 |
...
`GIT_OPTIONAL_LOCKS=0` 消除了**正确性**问题。剩余的是**资源**问题:用户没开 Git 面板时后台仍每 3s 轮询一次 status。**本期不做**,理由:
...
- (a) 我只 stage 并 commit 我的勘误那一批(推荐,commit message:`fix(git): use GIT_OPTIONAL_LOCKS=0 for status polling`),遗留那批留给你;
