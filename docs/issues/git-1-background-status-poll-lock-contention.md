...
**状态**:**根因已修**(2026-07-21,`--no-optional-locks`);一项资源优化暂缓(见下)
...
git diff,永不写 .git**」。但 `git status` 不带 `--no-optional-locks` 时**实际会写
`.git/index`**(就是这造成了 lock)——也就是说修复前 Marina 每天都在「偷偷写 .git」,
只是写完就释放、没造成数据损坏而已。这是对自家设计契约的**无意违规**。

## 已修(根因)

`git-service.ts` 所有 `git status` 调用点(共 3 处:`getStatus` / `getStatusInternal`
后台轮询路径 / `produceDiff` 单文件查)全部加上 `--no-optional-locks` flag。

- git 文档:`--no-optional-locks` = 「不执行需要锁的可选操作」,**专为后台轮询工具设计**。
- 效果:git 跳过 index refresh 的写回,**不再创建 `.git/index.lock`**;状态结果对只读
  展示完全不变(仍扫描工作区检测 modified/untracked),只是不持久化 index 缓存刷新。
- 同时让 Marina 严格符合「永不写 .git」契约。

回归保护:`git-service.test.ts` 的 getStatus 组装测试新增断言——所有 status 调用的
args 必须含 `--no-optional-locks`,此 flag 一旦被误删会立即测试失败。

验证:`typecheck` + `test`(857/857)+ `lint` 全过。

## 暂缓:watcher 绑定面板可见性(资源优化,非正确性)

`--no-optional-locks` 消除了**正确性**问题(不再抢锁/不再写 .git)。剩余的是**资源**
...
- `src/main/git-service.test.ts`:getStatus 组装测试 + `--no-optional-locks` 回归断言
...
- git 文档:`git help status` → `--no-optional-locks`
