...
> 文件，以及命令输出的 Markdown（`marina run` 的 tab，ADR-037 起与文件同在
> 「已打开」面板，命令 tab 排在文件 tab 之后），都在下面这套能力范围里。
>
...
触发 show/run、代码块可一键运行、图片可直接看图、还能做画廊和目录导航。这些能力
对两种来源（`marina show` 打开的文件 / 命令面板渲染的输出）**默认同享**；唯一差异
是相对路径的解析基准：文件相对 md 文件所在目录，命令输出相对**该命令运行时的
cwd**（终端后来 cd 走也不影响旧输出）。
...
- 相对路径基准 = **md 文件自己的目录**（不是终端 cwd）——md 文件移动后只要相对布局
  不变，链接仍然有效。
- **命令面板输出里同样可点**（v0.3.3 ADR-036 起）：相对路径基准 = **该命令运行时
  的 cwd**（不是点击时的 cwd，终端后来 cd 走不影响旧输出的链接）。
- 绝对路径直接生效，含 Windows 盘符形式：`[图](D:\a\b.png)`、`[页](C:/x/y.html)`
  （反斜杠/正斜杠都行；.html 在面板里按源码文本展示）。
- **裸路径自动可点**：正文里没有链接语法的盘符绝对路径（如 `read D:\a\b.png`）
  也会自动变成可点链接，点击同样在面板打开。含空格的路径不猜，请写成链接语法。
...
- `show` 的路径相对 **md 文件所在目录**（命令面板输出相对该命令运行时 cwd）；
...
- 命令面板输出里的本地图同样可用：相对**该命令运行时的 cwd** 解析（与本地
  链接同一基准），不再显示占位。
...
- Markdown 打开后**自动生成标题大纲**（窄轨道，hover 展开成可点目录）——
  命令面板输出同样有目录与折叠（ADR-036 起不再文件专属）。
- 层级标题可折叠成章节。
...
| **命令输出基准** | 本地链接/图/gallery 相对**命令运行时 cwd** 解析（v0.3.3 ADR-036 起可用，不再是文件专属）；SSH 会话的远端路径无法本地解析 |
