# AGENTS.md — EasyTerm 项目 AI Agent 工作说明书

> 这份文件是给即将构建 EasyTerm 的 AI agent(Claude Code / Cursor / Codex / 其他)看的。
...
你被授权在不打扰开发者的前提下,**端到端**完成 EasyTerm 的构建。这意味着:
...
* `git push --force` 任何操作
* 修改 `.git/` 目录
* 删除或修改 `~/AppData/Roaming/EasyTerm/` 下的任何文件(即使是测试)
* 卸载 npm 包
* 修改系统注册表(Windows)
 ...
 * Session 状态转移逻辑。
 *
 * 状态机参见 软件定义书.md 第 8.3 节。
 * 简化:
 *   active <--有/无字节流--> idle
 *   active/idle --进程退出--> tombstoned
 *   tombstoned --5 分钟到 / 用户关闭--> destroyed
 *   tombstoned --用户重启--> active(新 PTY)
 */
```
...
throw new Error(
  ...
  `Check logs in ~/AppData/Roaming/EasyTerm/logs/main.log for stderr.`
);
```
...
EasyTerm 的开发被切分成 **N 个检查点**,与 `软件定义书.md` 第 15 章的 Phase 1 路线图对齐。
...
- [ ] 关闭窗口 → 应用进入纯托盘模式(托盘图标还在,任务管理器里 EasyTerm.exe 还在)
- [ ] 单击托盘图标 → 重新打开一个窗口(窗口编号变了)
- [ ] 右键托盘 → 看到菜单,菜单里"完全退出"能真正退出应用
- [ ] 启动第二次 EasyTerm.exe → 在已运行实例上新开一个窗口(单实例锁工作)
...
**完成标志**:
- [ ] 5 套主题都可切换,即时生效,xterm 颜色与 UI 同步
...
- [x] 关闭窗口后,任务管理器查看 EasyTerm.exe 仍在
- ...

## 已知不工作的事(需要开发者关注)
- 第二次启动 EasyTerm.exe 时,新窗口的位置和第一个重叠了 → 这个属于 CP2 的窗口位置记忆功能,目前先不修
- ...
...
## 测试 1:基础启动(预计 1 分钟)
1. 跑 `npm run dev`
2. 等待 ~10 秒,应该看到一个 EasyTerm 窗口出现
3. 窗口右半部分应有一个黑色终端区域,显示 PowerShell 提示符,如 `PS C:\Users\xxx>`
4. **预期**:你能在终端里输入 `dir` 并看到当前目录列表
5. **失败现象**:窗口空白 / 终端显示乱码 / 终端输入无反应
6. **失败时**:打开 `~/AppData/Roaming/EasyTerm/logs/main.log`,把最后 50 行贴给 agent

## 测试 2:窗口与应用解耦(预计 30 秒)
1. 关闭那个窗口(点右上角 ×)
2. 看 Windows 系统托盘(右下角),应该有一个 EasyTerm 图标
3. 打开任务管理器,搜索 "EasyTerm",应该看到 EasyTerm.exe 进程仍在
4. **预期**:窗口关了,但应用还在
5. 单击系统托盘的 EasyTerm 图标
...
* Session 状态机的所有转移(active ↔ idle ↔ tombstoned ↔ destroyed)
...
测试 / 调试 / 开发过程中,**永远不要**:
* 删除 `~/AppData/Roaming/EasyTerm/` 下任何文件
* 修改 `~/AppData/Roaming/EasyTerm/` 下任何文件(除非是应用本身的正常写入)
* 用真实的 EasyTerm 数据目录跑测试

测试用临时目录或内存文件系统:
```typescript
// 测试里这么用
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'easyterm-test-'));
// 测试结束清理
...
11. 收到 "CP-1 xxx 不工作" → 修复,重新自测,再标记
12. 重复直到 CP-5 完成
13. 输出 🎉 ALL CHECKPOINTS COMPLETE: EasyTerm V1 构建完成
```
...
1. **你的工作不是"写出最优雅的代码"**,而是"实现一个可用的 V1 EasyTerm,让开发者接手后能维护"。
...
| `.git/` | ✅ | ❌ | ❌ |
| `~/AppData/Roaming/EasyTerm/` | ⚠️ 仅 EasyTerm 应用代码 | ❌(测试不许碰) | ❌(永远不许) |
...
> 这份说明书会随 EasyTerm 项目演化而修订。如果你看的版本是 1.0,而 git 里有更新版本,以最新版本为准。
>
