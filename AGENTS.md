...
>
> 文档版本:1.12 · 本文件的版本变更历史见 `docs/AGENTS-changelog.md`。
...
YOLO 不是"想干啥干啥"。以下四条边界**永远不能越过**,越过即停止:
...
#### 边界 4:版本号变更(v1.12,2026-09-17 裁决)

**版本号的变更(正式 bump、开新版本线、预判 MINOR/PATCH 目标)由开发者决定,AI 不得自作主张。** AI 在版本号上唯一能自主做的事:在开发者**已定的目标版本号**下迭代 dev 构建编号(`-dev.N` 的 N 递增)。

* 需要产 dev 构建(出包测试)而目标版本未定 → **停下来问开发者这批归哪个版本线**,不要按"改动量级口诀"自行预判
* 发现版本状态与自己的理解冲突(如 CHANGELOG 已折叠、tag 已存在)→ 这**不构成**自行判断"已发版/该开新线"的依据,问开发者
* 正式 bump(去 `-dev.N` 后缀发版)→ 开发者明示后才执行

理由(v1.12 前车之鉴):agent 曾从"CHANGELOG 有带日期的 [0.4.0] 条目 + 存在 v0.4.0 tag"推导出"0.4.0 已正式发版",进而自行预判 MINOR 开出 0.5.0-dev.1——实际 0.4.0 一直是开发态。版本的"开发多久、攒没攒够、要不要开新线"是产品节奏判断,必须人来做。
...
| 决定版本号 bump / 产出 dev 构建 | `docs/版本号规则.md`(原附录 E、F)。**裁决权:版本号变更归开发者,AI 只能迭代 dev.N(边界 4)** |
...
| 查 AGENTS.md 历史版本说明(v1.1~v1.12) | `docs/AGENTS-changelog.md` |
